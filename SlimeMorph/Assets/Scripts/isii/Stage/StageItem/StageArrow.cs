...
public class StageArrow : StageObjectItem
{
    IT_PlayerPosCheck playerPosCheck;
    float speed = 10f; // 障害物の移動速度

    [Header("障害物が動き始めるマス")]
    [SerializeField] private int firstMoveMas = 2;
    [Header("当たり判定系統")]
    ...
    void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.CompareTag("Player") && !isMorphed)
        {
            ...
            if (player == null)
            {
                Debug.LogWarning($"StageArrow: IT_Player not found on trigger target {other.name}", other);
                return;
            }
